<template>
<v-menu ref="menu1" v-model="menu1" :close-on-content-click="false" :return-value.sync="dataValor" transition="scale-transition" min-width="290px">
    <template v-slot:activator="{ on }">
        <slot :item="{ label, dataValor, on }">
            <v-text-field filled v-model="dataValor" :label="label" prepend-icon="event" readonly v-on="on" />
        </slot>
    </template>

    <v-date-picker v-model="dataValor" no-title scrollable width="390" color="blue darken-3" show-current first-day-of-week="1" :max="maxData" :min="minData" locale="pt" @input="
        menu1 = false;
        dataSelecionada();
      " />
</v-menu>
