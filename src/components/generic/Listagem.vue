    ...
    <v-row>
        ...
        <v-col class="py-0 my-0">
            ...
            <v-card flat style="border-radius: 10px !important;">
                ...
                <div class="info-content mx-12 mb-8" :class="{
              'margin-mdUp': $vuetify.breakpoint.mdAndUp,
              'margin-smDown': $vuetify.breakpoint.smAndDown
            }">
                    <v-tooltip top color="info" open-delay="500">
                        <template v-slot:activator="{ on }">
                            <v-text-field v-model="search" v-on="on" label="Filtrar" style="text-align: center !important;" class="centered-input mt-n1 mb-2 px-8" single-line clearable hide-details></v-text-field>
                        </template>
                        <span>Filtrar {{ tipo }}</span>
                    </v-tooltip>
                </div>
                <v-card-text>
                    <v-data-table class="mx-8 mb-8 pa-4" id="list-table" :headers="headers" :items="lista" :search="search" :footer-props="footer_props" v-if="this.headers[this.cabecalho.length - 1]">
                        <template v-slot:no-results>
                            <v-alert :value="true" color="error" icon="warning" class="font-weight-medium my-3" id="alerta-erro">Não foram encontrados resultados para
                                <b>"{{ search }}"</b>.
                            </v-alert>
                        </template>

                        <template v-slot:item="props">
                            <ListagemTI v-if="tipo === 'Termos de Índice'" :item="props.item" @rowClicked="go($event.idClasse)" style="cursor: pointer;" />

                            <ListagemTE v-else-if="tipo == 'Tipologias de Entidade'" :item="props.item" @rowClicked="go($event.id)" @iconClicked="
                    switchOperacao($event.operacao.descricao, $event.item.id)
                  " style="cursor: pointer;" />
                            <ListagemE v-else-if="tipo == 'Entidades'" :item="props.item" @rowClicked="go($event.id)" @iconClicked="
                    switchOperacao($event.operacao.descricao, $event.item.id)
                  " style="cursor: pointer;" />

                            <ListagemLegislacao v-else-if="tipo == 'Legislação'" :item="props.item" @rowClicked="go($event.numero)" @iconClicked="
                    switchOperacao($event.operacao.descricao, $event.item.id)
                  " style="cursor: pointer;" />
            ...
            " @click="$router.push('/rada/' + props.item.codigo)" style="cursor: pointer;">
              ...
              <div v-if="props.item">
                ...
                <div v-else>{{ campo }}</div>
                            ...
                            <tr v-else-if="tipo == 'Autos de Eliminação'" @click=" props.item.id.replace(/\//g, '_')" style="cursor: pointer;">
                                <td v-for="(campo, index) in props.item" v-bind:key="index">
                                    <div v-if="props.item">
                                        <div v-if="index === 'entidade'">
                                            <a :href="'/entidades/ent_' + campo">{{ campo }}</a>
