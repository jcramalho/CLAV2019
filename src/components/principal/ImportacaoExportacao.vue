    ...
    <v-expansion-panel-content>
        <v-card>
            <v-card-text v-for="colecao in this.colecoes" :key="colecao.nome">
                <p>{{ colecao.nome }}</p>
                <div v-if="level >= min">
                    <v-btn color="indigo accent-4" dark small class="ma-2" @click="go(`${colecao.importar}`)">Importar</v-btn>
                    <v-btn color="indigo accent-4" dark small class="ma-2" @click="exportarFicheiro(`${colecao.exportar}`)">Exportar</v-btn>
                </div>
            </v-card-text>
        </v-card>
    </v-expansion-panel-content>
