    ...
    <div>
        <v-layout row wrap justify-center>
            ...
            <v-flex xs11>
                ...
                <v-radio-group v-model="subdivisao" row >
        ...
        <v-layout v-if="subdivisao && motivoDF" row wrap justify-center>
            <v-flex xs11>
                ...
                <v-radio-group v-model="subclasse01Sintetiza02" col>
                    <v-radio v-bind:value="true" color="indigo">
                        <template v-slot:label>
                            <div>{{codigoClasse}}.01 sintetiza {{codigoClasse}}.02</div>
                        </template>
                    </v-radio>
                    <v-radio v-bind:value="false" color="indigo">
                        <template v-slot:label>
                            <div>{{codigoClasse}}.01 é sintetizada por {{codigoClasse}}.02</div>
                        </template>
                    </v-radio>
                </v-radio-group>
            </v-flex>
...
<script>
export default {
    props: ["subdivisao", "motivoPCA", "motivoDF", "subclasse01Sintetiza02","codigoClasse"],

    watch: {
        subdivisao: function(){
            this.$emit('subdivisao', this.subdivisao);
        },
        motivoPCA: function(){
            this.$emit('motivoPCA', this.motivoPCA);
        },
        motivoDF: function(){
            this.$emit('motivoDF', this.motivoDF);
        },
        subclasse01Sintetiza02: function(){
            this.$emit('subclasse01Sintetiza02', this.subclasse01Sintetiza02);
        }
    }
}
</script>
