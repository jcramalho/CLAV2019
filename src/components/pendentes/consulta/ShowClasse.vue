  ...
  <v-card class="mt-4">
    ...
    <v-card-text>
      ...
      <v-card
        class="ma-2"
        v-if="
          p.objeto.notasAp.length > 0 ||
            p.objeto.exemplosNotasAp.length > 0 ||
            p.objeto.notasEx.length > 0 ||
            (p.objeto.nivel == 3 && p.objeto.termosInd.length > 0)
        "
      >
        ...
        <v-card-text>
          <ShowNotasAp
            v-if="p.objeto.notasAp.length > 0"
            :lista="p.objeto.notasAp"
          />
          <ShowExemplosNotasAp
            v-if="p.objeto.exemplosNotasAp.length > 0"
            :lista="p.objeto.exemplosNotasAp"
          />
          <ShowNotasEx
            v-if="p.objeto.notasEx.length > 0"
            :lista="p.objeto.notasEx"
          />
          <ShowTIs
            v-if="p.objeto.nivel == 3 && p.objeto.termosInd.length > 0"
            :lista="p.objeto.termosInd"
          />
        </v-card-text>
      </v-card>
      ...
      <v-card class="ma-2" v-if="p.objeto.nivel >= 3">
        ...
        <v-card-text>
          ...
          <ShowParticipantes
            v-if="p.objeto.participantes.length > 0"
            :lista="p.objeto.participantes"
          />
          <ShowProcRel
            v-if="p.objeto.processosRelacionados.length > 0"
            :lista="p.objeto.processosRelacionados"
          />
          <ShowLegislacao
            v-if="p.objeto.legislacao.length > 0"
            :lista="p.objeto.legislacao"
          />
        </v-card-text>
      </v-card>
