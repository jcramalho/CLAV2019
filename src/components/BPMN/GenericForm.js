...
import * as FormTypes from '../pedidos';

export default Vue.component('GenericForm', {
  props: ['formKey', 'taskId', 'executionId', 'options'],
  data() {
    return {
      template: null
    };
  ...
  methods: {
    changeTemplate: function() {
      const formdata = FormTypes[this.formKey].data().formdata;
      if (formdata && this.taskId) {
        const variables = Object.keys(formdata);
        this.loadVariables(variables);
      }
      this.mycomponent = Vue.compile(`<div>
        <component ref="formsChild" :taskId="taskId" :options="options" :executionId="executionId" v-bind:is="formKey"></component>
      </div>`);
