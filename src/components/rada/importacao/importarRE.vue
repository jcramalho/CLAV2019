...
<v-dialog v-model="dialogState" max-width="80%">
    <v-card>
        <v-form ref="form">
            ...
            <v-card-actions>
                <v-alert width="100%" :value="erros_ficheiro" outlined type="error" prominent border="left">{{ erros.produtoras }}</v-alert>
                <v-spacer></v-spacer>

                <v-btn v-if="!importarCircle" color="indigo darken-4" dark @click="dialogState = false">Voltar</v-btn>
                <v-btn v-if="!importarCircle" color="indigo darken-4" dark @click="importar">Importar</v-btn>

                <v-progress-circular v-if="importarCircle" :size="40" :width="3" color="amber accent-3" indeterminate></v-progress-circular>
            </v-card-actions>
        </v-form>
    </v-card>
</v-dialog>
