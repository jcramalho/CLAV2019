...
<v-dialog v-model="dialogState" max-width="90%">
    <v-card>
        <v-card-title class="indigo darken-4 white--text">Consultar Classe {{ classe.tipo == "serie" ? "Série" : "Subsérie" }}:
            {{ classe.codigo }}
        </v-card-title>
        ...
        <v-card-text>
            ...
            <RADAEntryDouble label_1="Data Inicial" :value_1="classe.dataInicial" label_2="Data Final" :value_2="classe.dataFinal" />
            ...
            <div v-if="classe.tipo != 'subserie'">
                <RADAEntryDouble label_1="Tipo de Unidade Arquivística" :value_1="classe.tipoUA" label_2="Tipo de Série" :value_2="classe.tipoSerie" />
                <v-card outlined>
                    <RADAEntryDouble v-for="(valores, i) in classe.suporte_e_medicao" :key="i" :label_1="'Suporte (' + (i + 1) + ')'" :value_1="valores.suporte" :label_2="'Medição (' + (i + 1) + ')'" :value_2="valores.medicao" />
                </v-card>
                <RADAEntry label="Localização" :value="classe.localizacao" />
            </div>
            <v-expansion-panels>
                <v-expansion-panel class="ma-1">
                    <v-expansion-panel-header class="pa-2 indigo darken-4 title white--text">Zona de Contexto de Avaliação</v-expansion-panel-header>
                    <v-expansion-panel-content>
                        <RADAEntry label="Produtoras da Série" v-if="classe.tipo != 'subserie' && !!classe.produtoras[0]">
                            <template v-slot:valor>
                                <ul>
                          ...
                          produtora.ent_or_tip.split('#')[1].split('_')[0] ==
                            'ent'
                        " :href="'/entidades/ent_' + produtora.sigla">{{ produtora.sigla + " - " + produtora.designacao }}</a>
                        ...
                        <RADAEntry label="Legislação" v-if="classe.tipo != 'subserie' && !!classe.legislacao[0]">
                            <template v-slot:valor>
                                <ul>
                                    <li v-for="(legislacao, i) in classe.legislacao" :key="i">
                                        <a :href="'/legislacao/' + legislacao.leg.split('#')[1]">{{
                          "[" +
                            legislacao.tipo +
                            " " +
                            legislacao.numero +
                        ...
                        <RADAEntry label="Séries/Subséries Relacionadas" v-if="!!classe.relacoes[0]">
                            <template v-slot:valor>
                                <v-data-table :items-per-page="relacoes.length" locale="pt" :headers="headers" :items="relacoes" class="elevation-1" hide-default-footer>
                ...
                <v-expansion-panel class="ma-1" v-if="!!classe.pca">
                    <v-expansion-panel-header class="pa-2 indigo darken-4 title white--text">Zona de Decisões de Avaliação</v-expansion-panel-header>
                    <v-expansion-panel-content>
                        <br />
                        <v-card outlined>
                            ...
                            <v-card-text>
                                ...
                                <RADAEntry v-if="!!classe.pca.notaPCA" label="Nota sobre PCA" :value="classe.pca.notaPCA" />
                                <RADAEntry label="Forma de Contagem do PCA" :value="classe.pca.formaLabel"></RADAEntry>
                                <RADAEntry v-if="!!classe.pca.subformaLabel" label="Subforma de Contagem do PCA" :value="classe.pca.subformaLabel" />
                                <RADAEntry label="Justificação do PCA" v-if="!!classe.pca.justificacaoPCA[0]">
                                    <template v-slot:valor>
                                        <RADAEntry v-for="(criterio, cindex) in classe.pca.justificacaoPCA" :key="cindex" :label="
                        ...
                        ">
                                            <template v-slot:valor>
                                                ...
                                                <ul v-if="criterio.tipo == 'CriterioJustificacaoUtilidadeAdministrativa'">
                                                    <li v-for="(rel, i) in criterio.relacoes" :key="i">{{rel.codigo + " - " + rel.titulo}}</li>
                                                </ul>
                                                <ul v-else-if="
                              criterio.tipo == 'CriterioJustificacaoLegal'
                            ">
                        ...
                        <v-card outlined>
                            ...
                            <v-card-text>
                                ...
                                <RADAEntry v-if="!!classe.df.notadf" label="Nota sobre o DF" :value="classe.df.notadf" />
                                <RADAEntry label="Justificação do DF" v-if="!!classe.df.justificacaoDF[0]">
                                    <template v-slot:valor>
                                        <RADAEntry v-for="(criterio, cindex) in classe.df.justificacaoDF" :key="cindex" :label="
                          criterio.tipo ==
