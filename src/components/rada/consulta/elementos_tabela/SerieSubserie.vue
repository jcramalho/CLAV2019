...
<div>
    ...
    <v-stepper>
        <v-stepper-header>
            ...
            <v-stepper-step step="3" editable v-if="!classes.some(e => e.eFilhoDe == classe.codigo)">Zona de Decisões de Avaliação</v-stepper-step>
        </v-stepper-header>
        <v-stepper-items>
            <!-- zona descritiva -->
            <v-stepper-content :step="1" flat>
                <v-card flat>
                    <v-card-text>
                        ...
                        <RADAEntryDouble v-if="!!classe.dataInicial && !!classe.dataFinal" label_1="Data Inicial" label_2="Data Final" :value_1="classe.dataInicial" :value_2="classe.dataFinal" />
                        ...
                        <div v-if="classe.tipo == 'Série'">
                            <RADAEntryDouble v-if="!!classe.tUA && !!classe.tSerie" label_1="Tipo de Unidade Arquivística" :value_1="classe.tUA" label_2="Tipo de Série" :value_2="classe.tSerie" />
                            <v-card outlined v-if="!!classe.suporte_e_medicao[0].suporte && !!classe.suporte_e_medicao[0].medicao">
                            ...
                            <RADAEntry v-if="classe.localizacao.length > 0" label="Localização" :value="classe.localizacao.join('; ')" />
                        </div>
                    </v-card-text>
                    <br />
                </v-card>
            ...
            <v-stepper-content :step="2" flat>
                <v-card flat>
                    <v-card-text>
                        <div v-if="classe.tipo == 'Série'">
                            <RADAEntry label="Produtoras" v-if="
                  ...
                  ">
                        ...
                        <RADAEntry label="Séries/Subséries Relacionadas" v-if="!!classe.relacoes[0]">
                            <template v-slot:valor>
                                <v-data-table :items-per-page="classe.relacoes.length" locale="pt" :headers="headers" :items="classe.relacoes" class="elevation-1" hide-default-footer>
                    ...
                    <v-card outlined>
                        ...
                        <v-card-text>
                            ...
                            <div v-if="!!classe.formaContagem">
                                <RADAEntry v-if="!!classe.formaContagem.forma" label="Forma de Contagem do PCA" :value="forma(classe.formaContagem.forma)" />
                                <RADAEntry v-if="!!classe.formaContagem.subforma" label="Subforma de Contagem do PCA" :value="classe.formaContagem.subforma" />
                            </div>
                            <RADAEntry label="Justificação do PCA" v-if="
                    !!classe.justificacaoPCA &&
                      classe.justificacaoPCA.length > 0
                  ">
                                <template v-slot:valor>
                                    <RADAEntry v-for="(criterio, cindex) in classe.justificacaoPCA" :key="cindex" :label="criterio.tipo">
                                        <template v-slot:valor>
                                            {{ criterio.nota }}
                                            <ul v-if="
                            criterio.tipo ==
                    ...
                    <v-card outlined>
                        ...
                        <v-card-text>
                            ...
                            <RADAEntry label="Justificação do DF" v-if="
                    !!classe.justificacaoDF && classe.justificacaoDF.length > 0
                  ">
                                <template v-slot:valor>
                                    <RADAEntry v-for="(criterio, cindex) in classe.justificacaoDF" :key="cindex" :label="criterio.tipo">
                                        <template v-slot:valor>
                                            {{ criterio.nota }}
