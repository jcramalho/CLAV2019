...
<div v-else>
    <v-card class="ma-4" style="background-color: #fafafa">
        <v-card-title class="indigo darken-4 white--text">Relatório de Avaliação de Documentação Acumulada:
            {{ rada.titulo }}
        </v-card-title>
        <v-card-text>
            ...
            <v-expansion-panels>
                <v-expansion-panel class="ma-1">
                    <v-expansion-panel-header class="pa-2 indigo darken-4 title white--text">Relatório Expositivo</v-expansion-panel-header>
            ...
            for (var i = 0; i < this.rada.tsRada.length; i++) {
                if (this.rada.tsRada[i].pai == null) {
                    myTree.push({
                        ...
                        tipo: this.rada.tsRada[i].tipo == "funcional" ?
