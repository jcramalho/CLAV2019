...
<v-card class="ma-4" style="background-color:#fafafa">
    <v-card-title class="indigo darken-4 white--text">Criar Relatório de Avaliação de Documentação Acumulada</v-card-title>
    <v-card-text>
        <br />
        ...
        <v-stepper v-model="e1" vertical class="elevation-0" style="background-color:#fafafa">
            ...
            <v-stepper-content step="2">
                <RelatorioExpositivo @seguinte="changeE1" :classes="RADA.tsRada.classes" :UIs="RADA.tsRada.UIs" :RE="RADA.RE" @guardar="guardarTrabalho" :entidades="entidades" :entidadesProcessadas="entidadesProcessadas" :tipologias="tipologias" />
            </v-stepper-content>
            ...
            <v-stepper-content step="3">
                <TSRada @done="concluir(idPendente)" @voltar="changeE1" @guardar="guardarTrabalho" @remover="toDelete = true" @validar="validarRADA" :legislacao="legislacao" :RE="RADA.RE" :TS="RADA.tsRada" :entidades="entidades" :legislacaoProcessada="legislacaoProcessada" :loading_circle.sync="loading_circle_ts" :pode_remover="pode_remover" />
                <v-alert width="100%" :value="!!erroProdutoras[0] || !!erros_relacoes[0] || !!erros_datas_uis[0] || !existe_serie || !!erros_em_falta[0]" outlined type="error" prominent border="left">
                    ...
                    <div v-if="!!erroProdutoras[0]">
                        <b>As seguintes tipologias/entidades produtoras não foram
                    ...
                    <div v-if="!!erros_relacoes[0]">
                        <b>As seguintes relações entre classes são inválidas devido às
                            datas:</b>
                        <ul>
                    ...
                    <div v-if="!!erros_datas_uis[0]">
                        <b>As seguintes associações entre classes e unidades de instalação
                    ...
                    <div v-if="!existe_serie">
                        <b>Deve adicionar séries ao RADA, antes de o submeter. Tem
                            possibilidade de associar unidades de instalação às séries em
                            avaliação.</b>
                    </div>
                </v-alert>
                <v-alert width="100%" :value="alert_valido" outlined type="success" prominent border="left">Validação efetuada com sucesso!</v-alert>
            </v-stepper-content>
        </v-stepper>
        ...
        <v-row>
            <v-dialog v-model="toDelete" width="50%">
                <v-card>
                    <v-card-title class="headline grey lighten-2" primary-title>Pretende mesmo eliminar o trabalho?</v-card-title>
