        ...
        <v-card-text>
            <v-form ref="formUI" :lazy-validation="false">
                <v-row>
                    ...
                    <v-col sm="3" md="3">
                        <v-text-field :rules="[v => verificaCodigoUI(v) || !!v || 'Campo de preenchimento obrigatório!']" solo clearable v-model="UI.codigo" label="Código"></v-text-field>
                    </v-col>
                    <v-col md="3" sm="3">
                        <div class="info-label">Titulo</div>
                    </v-col>
                    <v-col sm="3" md="3">
                        <v-text-field :rules="[v => !!v || 'Campo de preenchimento obrigatório!']" solo clearable v-model="UI.titulo" label="Título"></v-text-field>
                    </v-col>
                </v-row>
                <v-row>
                    <v-col md="2" sm="2">
                    ...
                    <v-col sm="6" md="6">
                        <v-text-field solo clearable v-model="UI.descricao" label="Descrição"></v-text-field>
                    </v-col>
                </v-row>
                <v-row>
                    <v-col xs="12" sm="3">
                    ...
                    <v-col xs="12" sm="3">
                        <SelecionarData label="Data Inicial" :d="UI.dataInicial" @dataSelecionada="UI.dataInicial = $event" :dataMinima="RE.dataInicial" :dataMaxima="RE.dataFinal">
                            <template v-slot:default="slotProps">
                                <v-text-field :rules="[v => !!v || 'Campo obrigatório!']" v-model="slotProps.item.dataValor" :label="slotProps.item.label" prepend-icon="event" readonly v-on="slotProps.item.on" solo></v-text-field>
                            </template>
                        </SelecionarData>
                    </v-col>
                    <v-col xs="12" sm="3">
                    ...
                    <v-col xs="12" sm="3">
                        <SelecionarData label="Data Final" :d="UI.dataFinal" @dataSelecionada="UI.dataFinal = $event" :dataMinima="RE.dataInicial" :dataMaxima="RE.dataFinal">
                            <template v-slot:default="slotProps">
                                <v-text-field :rules="[v => data_final_valida(v, UI) || 'Campo obrigatório!']" v-model="slotProps.item.dataValor" :label="slotProps.item.label" prepend-icon="event" readonly v-on="slotProps.item.on" solo></v-text-field>
                            </template>
                        </SelecionarData>
                    </v-col>
                </v-row>
          ...
          {{UI.classesAssociadas}}-->
                <v-row>
                    ...
                    <v-col md="8" sm="8">
                        <v-data-table :headers="headers" :items="UI.classesAssociadas" hide-default-footer v-if="!!UI.classesAssociadas[0]">
                            <template v-slot:item.codigo="props">
                                <img v-if="props.item.tipo == 'Série'" style="width:23px; height:30px" :src="svg_sr" />
                                <img v-else-if="props.item.tipo == 'Subsérie'" style="width:23px; height:30px" :src="svg_ssr" />
                        ...
                        <v-alert dense v-else :value="true" icon="warning" color="amber accent-3">Não tem séries/subséries selecionadas...</v-alert>
                    </v-col>
                </v-row>
                <v-card outlined>
                        ...
                        <v-form ref="addRel" :lazy-validation="false">
                                ...
                                <v-col sm="2" xs="12">
                                    <v-select :rules="[v => !!v || 'Campo obrigatório para associar série/subsérie!']" :disabled="iscodvalido" label="Série / Subsérie" v-model="tipoClasse" :items="['Série', 'Subsérie']" chips solo clearable>
                                        <template v-slot:selection="data">
                                            <v-chip>
                                                <v-avatar left>
                                                    <img v-if="data.item == 'Série'" style="width:23px; height:30px" :src="svg_sr" />
                                                    <img v-if="data.item == 'Subsérie'" style="width:23px; height:30px" :src="svg_ssr" />
                                                </v-avatar>
                                                {{ data.item }}
                                            </v-chip>
                                        </template>
                            ...
                            <v-row>
                                <v-col cols="12" class="text-right">
                                    <v-btn @click="adicionarClasseUI(UI)" color="green lighten-1" style="margin-left: 10px" dark>Clique para associar série/subsérie à unidade de instalação</v-btn>
