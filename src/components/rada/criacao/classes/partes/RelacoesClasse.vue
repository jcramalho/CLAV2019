...
<div>
            ...
            <v-row>
                ...
                props.item.relacao
                        ...
                        <template v-slot:item.serieRelacionada="props">
                            <img v-if="props.item.serieRelacionada.tipo == 'Série'" style="width:23px; height:30px" :src="svg_sr" />
                            <img v-else-if="props.item.serieRelacionada.tipo == 'Subsérie'" style="width:23px; height:30px" :src="svg_ssr" />
            ...
            <v-card outlined>
                ...
                <v-card-text>
                    ...
                    <v-form ref="addRel" :lazy-validation="false">
                        <v-row>
                            <v-col sm="6" xs="6">
                                <v-combobox :rules="[v => codigoIgual(v)]" v-model="codrel" :items="getCodigos" label="Código" item-value="codigo" item-text="searchField" solo clearable chips :return-object="false">
                            ...
                            <v-col sm="6" xs="6">
                                <v-text-field :disabled="iscodvalido" :rules="[v => !!v || 'Campo Obrigatório']" v-model="tituloClasse" label="Título" solo clearable></v-text-field>
                            </v-col>
                        </v-row>

                        <v-row justify="center">
                            <v-col sm="6" xs="12">
                                <v-autocomplete :rules="[v => !!v || 'Campo obrigatório!']" v-model="rel" :items="filtrarRelacoes" label="Relação" solo clearable chips>
                                    <template v-slot:no-data>
                                        <v-list-item>
                                            <v-list-item-title>
                            ...
                            <v-col sm="6" xs="12">
                                <v-select :disabled="iscodvalido" :rules="[v => !!v || 'Campo obrigatório!']" label="Série / Subsérie" v-model="tipoClasse" :items="['Série', 'Subsérie']" chips solo clearable>
                        ...
                        <v-row>
                            <v-col cols="12" class="text-right">
                                <v-btn style="margin-left: 10px" dark color="green lighten-1" @click="add()">Clique para adicionar a relação com a série/subsérie</v-btn>
    ...
    <ShowSerieSubserie v-if="show_serie_subserie" :dialog="show_serie_subserie" @fecharDialog="show_serie_subserie = false" :formaContagem="formaContagem" :show_a_partir_de_pedido="false" :treeview_object="treeview_object" :classes="classes" />
</div>
</template>
        ...
        showClasse(item) {
            if (
                ...
                this.treeview_object = {
                    ...
                    titulo: item.serieRelacionada.codigo + " - " + item.serieRelacionada.titulo,
                    eFilhoDe: "",
                    children: [],
                };
