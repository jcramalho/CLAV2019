...
<v-card flat class="mb-12" style="background-color:#fafafa">
    <v-form ref="form" :lazy-validation="false">
        ...
        <div v-if="RE.tipologiasProd == null">
            <NovaEntidade :entidades="entidades" :produtoras="RE.entidadesProd" :tipologias="tipologias" :entidadesProcessadas="entidadesProcessadas" />
            <v-row>
                <v-col cols="12" xs="12" sm="3">
                    <div class="info-label">Entidades Produtoras</div>
                </v-col>
                <v-col xs="12" sm="9">
                    <v-autocomplete :rules="[v => !!v[0] || 'Campo de preenchimento obrigatório!']" v-model="RE.entidadesProd" :items="entidadesProcessadas" item-text="entidade" item-value="entidade" placeholder="Selecione as Entidades Produtoras" multiple solo>
                  ...
                  " @click:close="removeEnt(data.item)">{{ data.item.entidade }}</v-chip>
                        </template>
                    </v-autocomplete>
                </v-col>
            </v-row>
        ...
        <v-row v-if="!RE.entidadesProd[0]">
            ...
            <v-col xs="12" sm="9">
                <v-autocomplete :rules="[v => !!v || 'Campo de preenchimento obrigatório!']" v-model="RE.tipologiasProd" :items="tipologias" item-text="tipologia" item-value="tipologia" placeholder="Selecione a Tipologia de Entidade" solo :disabled="produtoraTipologiaClasse(RE.tipologiasProd)" chips deletable-chips>
                    <template v-slot:no-data>
                        <v-list-item>
                            <v-list-item-title>
        ...
        <v-row>
            ...
            <v-col xs="12" sm="9">
                <SelecionarData :d="RE.dataInicial" label="Data Inicial" @dataSelecionada="RE.dataInicial = $event" data-minima="1200-01-01">
                    <template v-slot:default="slotProps">
                        <v-text-field :disabled="bloquearData()" :rules="[v => !!v || 'Campo de preenchimento obrigatório!']" v-model="slotProps.item.dataValor" :label="slotProps.item.label" prepend-icon="event" readonly v-on="slotProps.item.on" solo></v-text-field>
                    </template>
                </SelecionarData>
            </v-col>
            <v-col cols="12" xs="12" sm="3">
            ...
            <v-col xs="12" sm="9">
                <SelecionarData :d="RE.dataFinal" label="Data Final" @dataSelecionada="RE.dataFinal = $event" data-minima="1200-01-01">
                    <template v-slot:default="slotProps">
                        <v-text-field :disabled="bloquearData()" :rules="[
                  v =>
                    data_final_valida(v) ||
            ...
            <v-expansion-panel class="ma-1" popout focusable>
                ...
                <v-expansion-panel-content>
                    <v-row>
                        ...
                        <v-col xs="12" sm="9">
                            <v-textarea solo name="input-7-4" v-model="RE.hist_admin" placeholder="História Administrativa/Biográfica" :rules="basicRule"></v-textarea>
                        </v-col>
                    </v-row>
                    <v-row>
                        <v-col cols="12" xs="12" sm="3">
                        ...
                        <v-col xs="12" sm="9">
                            <v-textarea solo name="input-7-4" v-model="RE.hist_cust" placeholder="História Custodial" :rules="basicRule"></v-textarea>
                        </v-col>
                    </v-row>
                </v-expansion-panel-content>
            </v-expansion-panel>
            <v-expansion-panel class="ma-1" popout focusable>
                ...
                <v-expansion-panel-content>
                    <v-row>
                        ...
                        <v-col xs="12" sm="9">
                            <v-textarea solo name="input-7-4" v-model="RE.sist_org" placeholder="Sistema de Organização" :rules="basicRule"></v-textarea>
                        </v-col>
                    </v-row>
                </v-expansion-panel-content>
            </v-expansion-panel>
            <v-expansion-panel class="ma-1" popout focusable>
                ...
                <v-expansion-panel-content>
                    <v-row>
                        ...
                        <v-col xs="12" sm="9">
                            <v-textarea solo name="input-7-4" v-model="RE.localizacao" placeholder="Localização" :rules="basicRule"></v-textarea>
                        </v-col>
                    </v-row>
                    <v-row>
                        <v-col cols="12" xs="12" sm="3">
                        ...
                        <v-col xs="12" sm="9">
                            <v-textarea solo name="input-7-4" v-model="RE.est_conser" placeholder="Estado de Conservação" :rules="basicRule"></v-textarea>
                        </v-col>
                    </v-row>
                </v-expansion-panel-content>
            </v-expansion-panel>
        </v-expansion-panels>
        <ImportarRE :dialog="importar_re" @fecharDialog="importar_re = false" :entidades="entidades" :tipologias="tipologias" :RE="RE" />
    </v-form>
    <br />
