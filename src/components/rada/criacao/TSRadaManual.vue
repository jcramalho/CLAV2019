...
<v-card flat class="mb-12" style="background-color:#fafafa">
    <v-row>
        ...
        <v-col xs="12" sm="9">
            <v-text-field :rules="[v => !!v || 'Campo de preenchimento obrigatório!']" v-model="TS.titulo" label="Título da Tabela de Seleção" solo clearable></v-text-field>
        </v-col>
    </v-row>
    <v-row>
        <v-col cols="auto">
    ...
    <AddOrgFunc :dialog="criar_area" v-if="criar_area" @fecharDialog="criar_area = false" :classes="TS.classes" />
    <!-- v-if="criar_serie" -->
    <Serie :dialog="criar_serie" @fecharDialog="
        criar_serie = false;
        classe_copia = null;
      " @limpar_copia="classe_copia = null" :classe_para_copiar="classe_copia" :classes="TS.classes" :legislacao="legislacao" :RE="RE" :UIs="TS.UIs" :formaContagem="formaContagem" :legislacaoProcessada="legislacaoProcessada" :tipos="tipos" />
    <SubSerie :dialog="criar_subserie" :classe_para_copiar="classe_copia" @fecharDialog="
        criar_subserie = false;
        classe_copia = null;
      " @limpar_copia="classe_copia = null" :classes="TS.classes" :UIs="TS.UIs" :formaContagem="formaContagem" :RE="RE" />
    <!-- {{ TS.classes }} -->
    <v-row v-if="!tree_ou_tabela">
        <v-col cols="12" xs="12" sm="12">
            <div v-if="TS.classes.length > 0">
                <v-treeview hoverable :items="preparaTree" item-key="codigo" :search="search" :filter="filter">
                  ...
                  " @click="nova_classe_copia(item)" style="padding-left: 15px;" small>file_copy</v-icon>
                        </div>
                    </template>
                </v-treeview>
                <br />
    ...
    <!-- IMPORTAR CLASSES -->
    <ImportarClasses v-if="importar_classes" :formaContagem="formaContagem" :dialog="importar_classes" :classes="TS.classes" :RE="RE" :legislacao="legislacaoProcessada" @fecharDialog="importar_classes = false" @pendurarNovasClasses="mergeClasses" />
    <!-- CRIAR CLASSES -->
    <EditarSerie v-if="editar_serie" :dialog="editar_serie" @fecharDialog="editar_serie = false" @atualizacao="atualizacao_serie" :treeview_object="treeview_object" :classes="TS.classes" :legislacao="legislacao" :legislacaoProcessada="legislacaoProcessada" :RE="RE" :UIs="TS.UIs" :formaContagem="formaContagem" @remover="remover_classe" :tipos="tipos" />
    <EditarSubserie v-if="editar_subserie" :dialog="editar_subserie" @fecharDialog="editar_subserie = false" @atualizacao="atualizacao_subserie" @remover="remover_classe" :treeview_object="treeview_object" :classes="TS.classes" :RE="RE" :UIs="TS.UIs" :formaContagem="formaContagem" />
    <EditarOrganicaFunc v-if="editar_area_organico" :dialog="editar_area_organico" @fecharDialog="editar_area_organico = false" @atualizacao="atualizacao_area_organico" @remover="remover_classe" :classes="TS.classes" :treeview_object="treeview_object" />

    <v-progress-circular v-if="loading_circle" :size="70" :width="3" color="amber accent-3" indeterminate></v-progress-circular>
    <div v-else>
        <v-row no-gutters>
            <v-col cols="12">
                ...
                <v-btn style="margin-left: 10px" color="indigo darken-4" dark @click="$emit('guardar', 'nao')">Continuar Depois</v-btn>
                <v-btn style="margin-left: 10px" color="indigo darken-4" @click="$emit('validar')">
                    <font style="color: white">Validar</font>
                </v-btn>
                <v-btn style="margin-left: 10px" :disabled="
              !Boolean(TS.classes[0]) ||
                UIs_validas ||
                incompleto ||
                !Boolean(TS.titulo)
            " color="indigo darken-4" @click="sendToFather()">
                    <font style="color: white">Submeter</font>
                </v-btn>

                <v-btn style="margin-left: 10px" color="red darken-4" dark v-if="pode_remover" @click="$emit('remover')">Eliminar</v-btn>
            </v-col>
        </v-row>
    </div>
</v-card>
            ...
            for (var i = 0; i < this.TS.classes.length; i++) {
                if (this.TS.classes[i].eFilhoDe == null) {
                    myTree.push({
                        ...
                        titulo: this.TS.classes[i].codigo + " - " + this.TS.classes[i].titulo,
