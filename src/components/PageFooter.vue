    ...
    <v-card flat>
        <v-card-title id="footer-bar" class="hidden-sm-and-down px-2 py-2">
            ...
            <v-btn text rounded href="mailto:clav@dglab.gov.pt" id="contact-button">
                <unicon name="contacto-icon" width="24" height="24" viewBox="0 0 25.71 19.282" fill="#e5e5e5" />
                <p class="icon-text">Contacte-nos</p>
            </v-btn>
            <v-spacer></v-spacer>
            <v-btn text rounded @click="$router.push('/colaboracoesInfo')" id="contact-button">
                <v-icon color="#e5e5e5">{{ equipatecnica }}</v-icon>
                <p class="icon-text">Equipa técnica</p>
            </v-btn>

            <v-spacer></v-spacer>
            <v-img id="footer-image" class="my-2" :src="require('./../assets/feder.png')" aspect-ratio="4.8600" />
        </v-card-title>
        <!--Mobile Footer-->
        <v-card-title id="footer-bar" class="hidden-md-and-up px-4 py-3">
            ...
            <v-spacer></v-spacer>
            <v-btn text rounded href="mailto:clav@dglab.gov.pt" id="contact-button" class="mt-3 pa-0">
                <unicon name="contacto-icon" width="24" height="24" viewBox="0 0 25.71 19.282" fill="#e5e5e5" />
                <p class="icon-text">Contacte-nos</p>
            </v-btn>
            <v-spacer></v-spacer>
            <v-btn text rounded @click="$router.push('/colaboracoesInfo')" id="contact-button" class="mt-3 pa-0">
                <v-icon color="#e5e5e5">{{ equipatecnica }}</v-icon>
                <p class="icon-text">Equipa técnica</p>
            </v-btn>
            <v-spacer></v-spacer>
            <v-img id="footer-image" class="mt-3" :src="require('./../assets/feder.png')" aspect-ratio="4.8600" />
        </v-card-title>
    </v-card>
</v-footer>
</template>
...
export default {
    data: function () {
        return {
            ...
            equipatecnica: mdiAccountGroupOutline
        };
    },
