        ...
        <v-col class="py-0 my-0">
            ...
            <v-card flat style="border-radius: 10px !important;">
                <p class="content-title-1 pt-5" style="color: #4da0d0 !important; text-align: center;  padding-bottom: 0.7rem !important;">
                    Nova Tabela de Seleção Pluriorganizacional (continuação do trabalho
                    guardado)
                </p>
                <!-- CONTENT -->
                <v-card-text class="mt-0">
                    <v-stepper v-model="stepNo" id="stepper-card" class="mx-8 mt-n8 pa-4">
                        <v-stepper-step :complete="stepNo > 1" step="1">
                            <font size="4"><b> Entidades abrangidas pela TS</b></font>
                            <span v-if="stepNo > 1">
                                <v-chip v-for="(e, i) in tabelaSelecao.entidades" :key="i" class="ma-2" color="indigo darken-4" text-color="white" label>
                        ...
                        <v-stepper-content step="2">
                            <v-col xs12 sm6 md10>
                                <v-form ref="nomeTS" :lazy-validation="false">
                                    <v-text-field placeholder="Designação da Nova Tabela de Seleção" v-model="tabelaSelecao.designacao" :rules="[(v) => !!v || 'A designação não pode ser vazia']"></v-text-field>
                                </v-form>
                                <v-btn color="indigo darken-4" class="white--text" @click="validaTSnome">Continuar</v-btn>
                            </v-col>
                        </v-stepper-content>
                        ...
                        <v-stepper-content step="3">
                            <v-col v-if="listaProcessosReady">
                                <v-card>
                                    <v-card-text>
                                        <ListaProcessos :listaProcs="listaProcessos" :listaCodigosEsp="listaCodigosEsp" />
                                    </v-card-text>
                                </v-card>
                            </v-col>
                            ...
                            <v-card-actions style="margin-left: 5px">
                                <v-btn v-if="stepNo > 2" color="indigo darken-4" class="white--text" @click="stepNo = 2">Voltar</v-btn>

                                <v-btn v-if="stepNo > 2" color="indigo darken-4" class="white--text" @click="validarTS">
                                    Validar TS
                                ...
                                <v-btn v-if="stepNo > 2" color="indigo darken-4" class="white--text" @click="guardarTrabalho()">Guardar trabalho
                                    <DialogPendenteGuardado v-if="pendenteGuardado" :pendente="pendente" @continuar="pendenteGuardado = false" />
                                </v-btn>

                                <v-btn v-if="stepNo > 2" color="indigo darken-4" class="white--text" @click="submeterTS">Submeter</v-btn>

                                <v-btn v-if="stepNo > 2" color="indigo darken-4" class="white--text" @click="sairOperacao = true">Sair
                                    <DialogSair v-if="sairOperacao" @continuar="sairOperacao = false" @sair="sair" />
                                </v-btn>

                                <v-btn dark color="red darken-4" @click="eliminarTabela = true">Cancelar
            ...
            for (let i = 0; i < p.termosInd.length; i++) {
                ...
                if (filtrados.length > 1) {
                    this.mensagensErro.push({
                        ...
                        mensagem: "[" + p.termosInd[i].termo + "] noutro processo selecionado."
                    });
                    this.numeroErros++;
                }
            }
