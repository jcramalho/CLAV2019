...
<div>
    ...
    <v-data-table :items="listaProcs.procs" :headers="headers" class="ma-1" item-key="chave" :footer-props="procsFooterProps" :search="filtro">
          ...
          backgroundColor: props.item.edited
              ...
              : (props.item.preSelected > 0 ? '#FFECB3' : 'transparent')
                ...
                <td>
                    ...
                    <v-btn v-if="props.item.descriptionEdited" small class="ma-2" @click="editaBlocoDescritivo(props.item)">
    ...
    <v-row wrap>
        <v-col>
            <v-text-field readonly label="Nº de processos selecionados" v-model="listaProcs.numProcessosSelecionados"></v-text-field>
        </v-col>
        <v-col>
            <v-text-field readonly label="Nº de processos a selecionar" v-model="listaProcs.numProcessosPreSelecionados"></v-text-field>
        </v-col>
    </v-row>
