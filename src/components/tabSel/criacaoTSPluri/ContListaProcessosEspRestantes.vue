                      ...
                      >
                        <v-card>
                          <v-card-title class="headline"
                            >Confirmar ação</v-card-title
                          >
                          <v-card-text
                            >Pretende eliminar esta participação?</v-card-text
                          >
                          <v-card-actions>
                            <v-spacer></v-spacer>
                            <v-btn
...
export default {
  ...
  data: () => ({
    // Cabeçalho da tabela para selecionar os PNs especificos restantes
    headers: [
      {
        text: "Classe",
        value: "classe",
    ...
    // Processos especificos restantes selecionados (como dono ou participante ou ambos)
    procEspResSel: [],
  ...
  methods: {
    ...
    calcRel: async function(processo) {
      try {
        ...
        // Coloca na lista de processos resultantes especificos restantes os processos pré selecionados
        // resultantes das travessias dos processos comuns e dos processos especificos
    ...
    guardaEntDonos: async function(proc, des) {
      for (var i = 0; i < Object.keys(this.entProcDono[proc]).length; i++) {
        ...
        if (this.entProcDono[proc][Object.keys(this.entProcDono[proc])[i]]) {
          ...
          if (!this.procEspResSel.find(x => x.classe === proc)) {
            this.procEspResSel.push({
              classe: proc,
              designacao: des
            });
      ...
      if (!haDono) {
        ...
        if (Object.keys(this.entProcPar[proc]).length == 0) {
          var index = this.procEspResSel.indexOf(x => x.classe === proc);
    ...
    guardaEntPar: async function(proc, des) {
      ...
      this.$emit("guardarTSProcRes", guardar);
      if (
        !this.procEspResSel.find(x => x.classe === proc) &&
      ...
      for (var i = 0; i < this.lista.length; i++) {
        ...
        for (j = 0; j < this.entidades.length; j++) {
          ...
          if (
            ...
            if (!this.procEspResSel.find(x => x.classe === this.lista[i].classe)) {
              this.procEspResSel.push({
