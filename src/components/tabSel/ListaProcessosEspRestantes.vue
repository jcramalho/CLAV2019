...
export default {
  ...
  data: () => ({
    ...
    snackbar: false,
    text: "Por favor aguarde enquanto são efetuados os calculos das travessias iniciais dos processos de negócio específicos restantes.",
    value: 0,
  }),
  methods: {
          ...
          for (var i = 0; i < listaResultados.length; i++) {
            ...
            for (var j = 0; j < this.lista.length; j++) {
              if (
                ...
                !this.listaResEspRestantes.includes(listaResultados[i].codigo)
              ) {
                this.listaResEspRestantes.push(listaResultados[i].codigo);
                procEspecifico = true;
                break;
              }
            }
