...
<div class="ma-2">
    ...
    <v-dialog v-model="successDialog" width="700" persistent>
        <v-card outlined>
            ...
            <v-card-text>
                <span class="subtitle-1" style="white-space: pre-wrap" v-html="success">
                </span>
            </v-card-text>
            ...
            <v-card-actions>
                ...
                <v-spacer />
                <v-btn color="green darken-4" text @click="$router.push(`/pedidos/submissao/${codigo}`)">
                    Seguir pedido
                </v-btn>
            </v-card-actions>
        </v-card>
    ...
    <v-card class="ma-2 mx-auto" outlined width="60%">
        ...
        <v-card-text>
            <v-text-field clearable v-model="designacao" label="Designação da Tabela de Seleção"></v-text-field>
            <v-select :items="['Organizacional', 'Pluriorganizacional']" label="Tipo de Tabela de Seleção" v-model="tipo" @change="entidade_tipologia = null">
            </v-select>
            <div v-if="tipo != null">
                <v-autocomplete v-if="tipo == 'Organizacional'" :items="entidades_tipologias" label="Entidade/Tipologia" v-model="entidade_tipologia" prepend-icon="account_balance">
                </v-autocomplete>
                <v-file-input v-model="file" label="Ficheiro CSV/Excel" placeholder="Selecione o ficheiro CSV/Excel com a Tabela de Seleção" show-size accept="text/csv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" multiple></v-file-input>
        ...
        <v-card-actions>
            <v-btn class="white--text ma-2" color="indigo darken-4" @click="cancelar()" :disabled="loading">
                Voltar
            </v-btn>
            <v-spacer />
            <v-btn class="white--text ma-2" color="indigo darken-4" @click="enviarFicheiro()" :loading="loading" :disabled="
            designacao == '' ||
              file.length == 0 ||
              tipo == null ||
