        ...
        <div class="info-content pa-4">
            ...
            <v-data-table class="content-table" v-if="tipo == 'TABELAS DE SELEÇÃO INSERIDAS NA CLAV'" :headers="headersTS" :items="lista" :search="search" :footer-props="footer_props">
                <template v-slot:no-results>
                    <v-alert :value="true" color="error" icon="warning" class="font-weight-medium my-3" id="alerta-erro">Não foram encontrados resultados para "{{ search }}".</v-alert>
                </template>
                    ...
                    <v-tooltip top color="info" open-delay="500">
                        <template v-slot:activator="{ on }">
                            <v-btn v-on="on" small text rounded @click="$router.push('/ts/' + item.id)">
                                <unicon name="look-icon" width="25" height="25" viewBox="0 0 20.71 15.574" fill="#0D47A1" />
                            </v-btn>
                        </template>
            ...
            <v-data-table class="content-table" v-else-if="tipo == 'RADA'" :headers="headers" :items="lista" :search="search" :footer-props="footer_props">
                <template v-slot:no-results>
                    <v-alert :value="true" color="error" icon="warning" class="font-weight-medium my-3" id="alerta-erro">Não foram encontrados resultados para "{{ search }}".</v-alert>
                </template>
                ...
                <template v-slot:[`item.link`]="{ item }">
                    ...
                    <v-tooltip top color="info" open-delay="500">
                        <template v-slot:activator="{ on }">
                            <v-btn v-on="on" small text rounded @click="$router.push('/pgd/' + item.idRADA)">
                                <unicon name="look-icon" width="25" height="25" viewBox="0 0 20.71 15.574" fill="#0D47A1" />
                            </v-btn>
                        </template>
                        <span>Ver RADA</span>
            ...
            <v-data-table class="content-table" v-else :headers="headers" :items="lista" :search="search" :footer-props="footer_props">
                <template v-slot:no-results>
                    <v-alert :value="true" color="error" icon="warning" class="font-weight-medium my-3" id="alerta-erro">Não foram encontrados resultados para "{{ search }}".</v-alert>
                </template>
                ...
                <template v-slot:[`item.link`]="{ item }">
                    ...
                    <v-tooltip top color="info" open-delay="500" v-if="item.idPGD">
                        <template v-slot:activator="{ on }">
                            <v-btn v-on="on" small text rounded @click="$router.push('/pgd/' + item.idPGD)">
                                <unicon name="look-icon" width="25" height="25" viewBox="0 0 20.71 15.574" fill="#0D47A1" />
                            </v-btn>
                        </template>
