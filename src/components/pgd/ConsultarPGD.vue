                ...
                <td :colspan="headers.length">
                  <v-card class="ma-1 elevation-0">
                    <v-card-text>
                      ...
                      <v-row
                        v-if="
                          item.designacaoDono || item.designacaoParticipante
                        "
                      >
  ...
  methods: {
    csvExport() {
      ...
      if (this.objeto.fonte.text == "RADA") {
        headers =
          "Código,N.º Referência,Título,Descrição,Diplomas Jurídico-Administrativo,PCA,Nota PCA,Forma de Contagem PCA,Justificação do PCA,DF,Nota DF,Justificação do DF";
        csvContent = [
          headers,
