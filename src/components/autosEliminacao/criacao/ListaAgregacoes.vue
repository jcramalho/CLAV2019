...
<div class="ma-1">
    ...
    <v-data-table :headers="(tipo=='TS_LC' || tipo=='PGD_LC') ? cabecalhoLC : cabecalho" :items="agregacoes" item-key="codigo" :items-per-page="5" class="elevation-1 ml-2 mt-3" :footer-props="footer_props" :search="search">
        <template v-slot:no-data>
            <tr>
                <td>
                    <v-text-field hint="Exemplo: AS_DGLAB_1/2019" label="Insira um código para a agregação" v-model="codigo" clearable></v-text-field>
                </td>
                <td>
                    <v-text-field hint="Exemplo: Auditoria à Entidade A" label="Insira um título para a agregação" v-model="titulo" clearable></v-text-field>
                </td>
                <td>
                    <v-text-field hint="Exemplo: 2009" label="Insira o ano de contagem do PCA" v-model="dataContagem" clearable></v-text-field>
                </td>
                <td v-if="tipo=='TS_LC' || tipo=='PGD_LC'">
                    <v-select :items="natureza" v-model="ni" dense></v-select>
                </td>
                <td>
                    <v-tooltip top>
        ...
        <template v-slot:item="prop">
            <tr v-if="prop.index === 0">
                ...
                <td>
                    <v-tooltip top>
                        <template v-slot:activator="{ on }">
                            <v-icon class="mx-2" v-on="on" small @click="limparAG">clear</v-icon>
                        </template>
            ...
            <tr>
                ...
                <td>
                    <v-tooltip top>
                        <template v-slot:activator="{ on }">
                            <v-icon v-on="on" small class="mr-2" @click="upAgregacao(prop.item)">arrow_upward</v-icon>
                        </template>
                        <span>Duplicar Agregação</span>
                    </v-tooltip>
                    <v-tooltip top>
                        <template v-slot:activator="{ on }">
                            <v-icon v-on="on" small @click="
                    deleteObj = prop.item;
                    deleteDialog = true;
                  ">delete</v-icon>
                        </template>
    ...
    <v-dialog v-model="deleteDialog" width="700" persistent>
        <v-card outlined>
            <v-card-title class="red darken-4 title white--text" dark>Eliminação de agregação</v-card-title>
    ...
    <v-dialog v-model="erroDialog" width="700" persistent>
        <v-card outlined>
            <v-card-title class="red darken-4 title white--text" dark>Erro: Não foi possível adicionar Agregação</v-card-title>
