      ...
      <v-card-text class="panel-body">
        ...
        <v-stepper v-model="steps" vertical>
          <v-stepper-step :complete="steps > 1" step="1"
            >Seleção de fonte e fundo</v-stepper-step
          >
          ...
          <v-stepper-step :complete="steps > 2" step="2"
            >Importação dos ficheiros de submissão</v-stepper-step
          >
          ...
          <v-stepper-content step="2">
            ...
            <v-btn
              ...
              :disabled="!fileSerie"
              v-if="fileSerie"
              >Validar Ficheiros e Continuar</v-btn
            >
            <v-btn class="ma-2" color="red darken-4" dark @click="cleanFiles"
              >Limpar</v-btn
            >
          </v-stepper-content>

          <v-stepper-step step="3"
            >Validação de classes / séries e agregações / unidades de
    ...
    <v-dialog v-model="errosValDialog" width="700" persistent>
      <v-card outlined>
        <v-card-title class="title" dark
          >Ficheiros anexo com {{ errosVal.numErros }} erros</v-card-title
        >
...
export default {
  ...
  methods: {
      ...
      for (var zc of this.auto.zonaControlo) {
        ...
        if (zc.notasPCA && !zc.validaNotaPCA) {
          this.errosVal.erros.push({
            sobre: "Notas do PCA",
            mensagem:
              'É necessária confirmação de cumprimento da nota do PCA "' +
              zc.notasPCA +
              '"',
          });
          this.errosVal.numErros++;
        ...
        if (zc.notaDF && !zc.validaNotaDF) {
          this.errosVal.erros.push({
            sobre: "Nota do DF",
            mensagem:
              'É necessária confirmação de cumprimento da nota do DF "' + zc.notaDF + '"',
          });
          this.errosVal.numErros++;
        }
        if (
          ...
          for (var just of zc.justificaDF)
            this.errosVal.erros.push({
              sobre: "Notas do PCA",
              mensagem:
                'É necessária confirmação de cumprimento da justificação do DF "' +
                just +
                '"\n',
            });
          this.errosVal.numErros++;
        ...
        for (var ag of zc.agregacoes) {
          ...
          if (res1 > currentTime.getFullYear()) {
            this.errosVal.erros.push({
              sobre: "Data Contagem",
              mensagem:
                "A Data de Contagem deve ser igual ou inferior à subtração do PCA ao ano corrente. Classe: " +
                zc.codigo +
                " - " +
                zc.referencia +
                " - Agregação: " +
          ...
          if (res2 < 0) {
            this.errosVal.erros.push({
              sobre: "Data Contagem",
              mensagem:
                "A Data de Contagem não pode ser inferior à Data de Início da Classe. Classe: " +
                zc.codigo +
                " - " +
                zc.referencia +
                " - Agregação: " +
    ...
    submit: async function () {
      for (var zc of this.auto.zonaControlo) {
        if (this.tipo == "PGD_LC" || this.tipo == "TS_LC") delete zc["referencia"];

        if (zc.destino === "Conservação") zc.destino = "C";
        else if (zc.destino === "Eliminação") zc.destino = "E";

        for (var ag of zc.agregacoes) if (zc.destino === "C") ag.ni = "Participante";
      }
          ...
          conversor(this.fileSerie, this.fileAgreg, this.tipo)
            .then(async (res) => {
              ...
              if (this.tipo == "TS_LC") {
                ...
                this.auto.zonaControlo.forEach((zc) => {
                  ...
                  if (!classe) {
                    ...
                    this.erro =
                      ...
                      "<p>O código <b>" +
                      zc.codigo +
                      "</b> introduzido não pertence a nenhuma junção entre código e referência nas classes presentes em " +
                      this.auto.legislacao.split(" - ")[0] +
                      ".</p>";
                    return; //ERROS
                  }
                ...
                this.auto.zonaControlo.forEach((zc) => {
                  var classe = this.classes.find((elem) => {
                  ...
                  if (!classe) {
                    ...
                    if (zc.codigo && zc.referencia)
                      this.erro =
                        ...
                        "</b> e Referência <b>" +
                        zc.referencia +
                        "</b> introduzidos não pertencem a nenhuma junção entre código e referência nas classes presentes em " +
                        this.auto.legislacao.split(" - ")[0] +
                        ".</p>";
                    else if (zc.codigo)
                      this.erro =
                      ...
                      this.erro =
                        ...
                        "<p>A Referência <b>" +
                        zc.referencia +
                        "</b> introduzida não pertence a nenhuma junção entre código e referência nas classes presentes em " +
                        this.auto.legislacao.split(" - ")[0] +
                        ".</p>";
                    return; //ERROS
                  } else if (
                    (this.tipo == "PGD" || this.tipo == "RADA") &&
                    classe.df.valor == "C"
                  ) {
                    this.flagAE = true;
                    this.erro =
                      "Classe / Série com " +
                      (zc.codigo ? "Codigo <b>" + zc.codigo + "</b> " : "") +
                      (zc.referencia
                        ? "Número de referência <b>" + zc.referencia + "</b> "
                        : "") +
    ...
    filtrarDonos: async function () {
      ...
      if (this.tipo == "TS_LC") {
        ...
        this.classes = response.data.tsRada
          .filter((c) => c.df && c.pca)
          .map((c) => {
            return {
