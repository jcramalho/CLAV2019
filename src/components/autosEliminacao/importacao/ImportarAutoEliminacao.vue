  ...
  <div>
      ...
      <v-card-text class="panel-body">
        <div class="ma-3">
          ...
          <li>
            ...
            <a
              :href="
                `${publicPath}documentos/Formulario_auto_importacao_classes_series.csv`
              "
              download
              >aqui</a
            >)
          ...
          <li>
            ...
            <a
              :href="
                `${publicPath}documentos/Formulario_auto_importacao_agregacoes_UI.csv`
              "
              download
              >aqui</a
            >)
    ...
    <v-dialog v-model="successDialog" width="950" persistent>
      <v-card outlined>
        <v-card-title class="teal darken-4 title white--text" dark
          >Pedido de importação de auto de eliminação submetido com
    ...
    <v-dialog v-model="valDialog" width="950" persistent>
      <v-card outlined>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="red darken-4" text @click="valDialog = false"
            >Voltar</v-btn
          >
          <v-btn
      ...
      for (var zc of this.auto.zonaControlo) {
        ...
        if (zc.notaDF && !zc.validaNotaDF) {
          this.errosVal.erros.push({
            sobre: 'Nota do DF',
            mensagem:
              'É necessária confirmação de cumprimento da nota do DF "' +
              zc.notaDF +
              '"',
          });
          this.errosVal.numErros++;
                  ...
                  if (!classe) {
                    ...
                    if (zc.codigo && zc.referencia)
                      this.erro =
                        ...
                        '</b> e Referência <b>' +
                        zc.referencia +
                        '</b> não foram encontrados em ' +
                        this.auto.legislacao.split(' - ')[0];
                    else if (zc.codigo)
                      this.erro =
                        'Codigo da classe <b>' +
                        zc.codigo +
                        '</b> não foi encontrado em ' +
                        this.auto.legislacao.split(' - ')[0];
                    else
                      this.erro =
                        'Referência <b>' +
                        zc.referencia +
                        '</b> não foi encontrada em ' +
                        this.auto.legislacao.split(' - ')[0];
                    return; //ERROS
                  } else if (
