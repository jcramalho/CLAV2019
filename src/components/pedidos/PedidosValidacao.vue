<template>
  <PainelCLAV titulo="Pedidos em Validação" infoHeader="Pedidos em Validação">
          ...
          <div class="info-content pa-4">
            ...
            >
              ...
              <template v-slot:[`item.tarefa`]="{ item }">
                ...
                <v-tooltip top color="info" open-delay="500" v-if="temPermissaoValidar()">
                  <template v-slot:activator="{ on }">
                    <v-btn v-on="on" small text rounded @click="validarPedido(item)">
                      <unicon
                        name="accept-icon"
                        width="25"
                        height="25"
  ...
  methods: {
    ...
    atualizaPedidos() {
      this.dadosTabela = this.pedidos.map((pedido) => {
        ...
        if (
          pedido.distribuicao[pedido.distribuicao.length - 1].proximoResponsavel !==
          undefined
        )
          dados.responsavel =
            pedido.distribuicao[pedido.distribuicao.length - 1].proximoResponsavel.nome;
