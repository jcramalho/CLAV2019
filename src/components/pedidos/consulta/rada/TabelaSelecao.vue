...
<v-card flat class="mb-12">
    ...
    <div v-if="!!TS.classes[0]">
        <v-row>
            <v-col xs="11" sm="11">
                <v-text-field v-model="searchClasse" label="Pesquise a classe" clearable append-icon="search"></v-text-field>
            </v-col>
            <v-col xs="1" sm="1">
                <v-tooltip top>
        ...
        <v-row v-if="!tree_ou_tabela">
            <v-col cols="12">
                <v-treeview hoverable :items="preparaTree" item-key="codigo" :search="searchClasse" :filter="filter">
        ...
        <v-row v-else>
            <TabelaClassesRADA :formaContagem="formaContagem" :classes="TS.classes" @editarClasse="showClasse" :search="searchClasse" />
        </v-row>
    </div>
    <div v-else>
        <br />
    ...
    <v-row v-if="TS.UIs[0]">
        <v-col cols="12" xs="12" sm="12">
            <v-text-field v-model="search" append-icon="search" label="Procurar Unidade de Instalação" single-line hide-details></v-text-field>
            <v-data-table :headers="headers" :items="filtrar_uis" :footer-props="footer_props" :items-per-page="5">
                ...
                <p v-else>Não tem séries/subséries associadas!</p>
                        ...
                        <td>
                            <v-list v-if="!!props.item.classesAssociadas[0]" dense>
                                <v-list-item v-for="(classe, i) in props.item.classesAssociadas" :key="i" align-center>
                                    <v-list-item-icon>
                                        <img v-if="classe.tipo == 'Série'" style="width:23px; height:30px" :src="svg_sr" />
    ...
    <ShowSerieSubserie v-if="show_serie_subserie" :dialog="show_serie_subserie" @fecharDialog="show_serie_subserie = false" :formaContagem="formaContagem" :treeview_object="treeview_object" :classes="TS.classes" :show_a_partir_de_pedido="true" />
    <ShowOrganico v-if="show_area_organico" @fecharDialog="show_area_organico = false" :dialog="show_area_organico" :treeview_object="treeview_object" :classes="TS.classes" />
