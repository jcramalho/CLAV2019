<template>
<div>
    <showPedidosDependentes v-if="!!p.pedidos_dependentes[0]" :pedidos="p.pedidos_dependentes" />
    <v-row>
        <v-col cols="12">
            <v-stepper vertical class="elevation-0">
                ...
                <v-stepper-content step="1">
                    <AnalisaInformacaoGeral :entidades="entidades" :RADA="p.objeto.dados" :novoHistorico="novoHistorico" />
                </v-stepper-content>
                ...
                <v-stepper-content step="3">
                    <AnalisaTS :RADA="p.objeto.dados" :novoHistorico="novoHistorico" :formaContagem="formaContagem" />
                </v-stepper-content>
            </v-stepper>
            <!-- <h1>RADAAA</h1>
        <p>{{ p.objeto }}</p>-->
    ...
    <v-row>
        <v-spacer />
        <PO operacao="Analisar" @avancarPedido="encaminharPedido($event)" @devolverPedido="despacharPedido($event)" v-if="fase == 'analise'" />
        <PO operacao="Validar" @finalizarPedido="verificaVermelhos($event)" @devolverPedido="despacharPedido($event)" v-else-if="fase == 'validacao'" :vai_para_despacho="true" />
    </v-row>
    <!-- Dialog de confirmação de operação -->
    <v-dialog v-model="dialogConfirmacao.visivel" width="50%" persistent>
        <ConfirmacaoOperacao :mensagem="dialogConfirmacao.mensagem" @fechar="dialogConfirmacao.visivel = false" @confirma="finalizarPedido(dialogConfirmacao.dados)" />
    </v-dialog>
</div>
</template>
...
import {
    converterParaTriplosRADA
} from "@/utils/conversorTriplosRADA";
    ...
    methods: {
        alterarOriginal() {
            ...
            Object.keys(this.novoHistorico.tsRada).map((e) => {
                if (e == "classes") {
                    for (
                        let i = 0; i < this.novoHistorico.tsRada.classes.dados.length; i++
                    ) {
