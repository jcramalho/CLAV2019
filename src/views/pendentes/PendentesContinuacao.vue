<template>
    <div>
        <ContinuaClasse v-if="objLoaded && tipoClasse" :obj="objeto.objeto" />
        <ContinuaPendente v-else-if="objLoaded" :obj="objeto" />
        <v-alert
            v-else
            type="warning"
        >
            Por algum motivo não foi possível carregar o trabalho pretendido.
...
export default {
    components: {
        ContinuaPendente, ContinuaClasse
    },
        ...
        axios
            ...
            .then(response => {
                ...
                switch(this.objeto.tipo){
                    case "Classe": this.tipoClasse = true;
                                    break;
                }
                this.objLoaded = true
            })
