var info = [
  ...
  for(var i=0; i < info.length; i++){
    for(var j=0; j < info[i].meta.levels.length; j++){
        var index = res.findIndex(elem => info[i].meta.levels[j] == elem.nivel);
        if(index!=-1){
            res[index].ops.push(info[i].path);
        }
        else{
            res.push({ nivel: info[i].meta.levels[j],
                ops: [info[i].path]
            });
        }
    }
