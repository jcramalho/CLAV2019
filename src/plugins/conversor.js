...
var excel2Json = function (file, tipo) {
    return new Promise(function(resolve,reject){
        ...
        .then(function(wb) {
            ...
            var auto = {
                tipo: tipo,
                entidade: wb.getWorksheet(1).getRow(1).getCell(2).text,
                fundo: wb.getWorksheet(1).getRow(3).getCell(2).text,
                zonaControlo: []
            }
            if(tipo === "PGD") auto.legislacao = "Portaria "+wb.getWorksheet(1).getRow(2).getCell(2).text
            else auto.legislacao = wb.getWorksheet(1).getRow(2).getCell(2).text
            ...
            autos.eachRow(function(row, rowNumber) {
                if(rowNumber > 1) {
                    ...
                    if(tipo === "PGD/LC") {
                        ...
                        var conservacao = row.getCell(3).text
                    } else {
                        auto.zonaControlo.push({
                            codigo: row.getCell(1).text,
